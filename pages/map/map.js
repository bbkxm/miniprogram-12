              ...
              success: (res) => {
                if (res.confirm) {
                  if (checkpointCheckins.length > 0) {
                    ...
                    wx.navigateTo({
                      url: `/pages/activityDetail/activityDetail?id=${this.data.activityId}&tab=checkins&checkpointId=${checkpoint.id}`
                    })
                  } else {
                    // 跳转到打卡页面
                    wx.navigateTo({
            ...
            wx.showModal({
              ...
              success: (res) => {
                if (res.confirm) {
                  // 跳转到活动详情页，并带上打卡点信息
                  wx.navigateTo({
                    url: `/pages/activityDetail/activityDetail?id=${activityInfo.id}&tab=checkins&checkpointId=${checkpointInfo.id}`
                  })
                }
              }
            })
